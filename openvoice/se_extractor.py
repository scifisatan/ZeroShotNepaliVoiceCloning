...
# The input voice by the user needs to be split into small segments
# This ensures that each segments are properly transcribed
# which increases accuracy and reduces memory usage

# we can train our own model for this as well

# We have two option for splitting the audio
# 1. Whisper model
# -> Based on the transcription of the audio,
#    the audio is split into segments, where
#    each segment is a sentence or a phrase

# 2. Voice Activity Detection (VAD)
# -> This method uses a VAD function to find where speech occurs
#    Collects these speech segments, concatenates them,
#    and then splits them into pieces of equal duration

# Actual se = speaker embedding, style embedding is extracted
# using voice_model.extract_se() function
...
def split_audio_whisper(audio_path, audio_name, target_dir='processed'):
    ...
    audio = AudioSegment.from_file(audio_path) # loads audio using pydub
    max_len = len(audio)
    ...
    #TODO how is segments being processed here?
...
def hash_numpy_array(audio_path):
    """
    Hashes the audio file to create a unique identifier for the audio file.
    """
